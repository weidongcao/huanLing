        ...
        with tf.variable_scope('mnist'):
            for gpu_idx in range(gpus):
                with tf.device('/gpu:%d' % gpu_idx):
                    ...
                    self.predict_s.append(tf.nn.softmax(x))

                    # 验证
                    x2 = tf.cast(tf.argmax(self.predict_s[-1], axis=1), tf.int32)
                    x2 = tf.equal(x2, y)
                    x2 = tf.cast(x2, tf.float32)
                    self.precise_s.append(tf.reduce_mean(x2))
            self.train_op = None
...
class Mnist:
    ...
    def predict(self):
        ...
        ds = input_data.read_data_sets('data/')
        imgs, labels = ds.test.next_batch(10)
...
app = flask.Flask(__name__, static_url_path='/home', static_folder='home')
mnist = None
...
def stop():
    mnist.stop()
    return 'The training is stopped'
...
def set_lr():
    ...
    if 'lr' not in params:
        return 'Param "lr" is not found...'
    lr = params['lr']
    try:
        lr = float(lr)
    except:
        return '%s is not a valid float' % lr
    mnist.set_lr(lr)
    return query()
